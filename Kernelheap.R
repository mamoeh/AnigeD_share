...
get_jaccard <- function(data, var) {
  
  hs_org <- data[data$version == "original", var]
  hs_low <- data[data$version == "low", var]
  hs_mid <- data[data$version == "mid", var]
  hs_hig <- data[data$version == "high", var]
...
ptab_low <- ptable::create_cnt_ptable(D = 4,  V = 2.5, js = 2) # small perturbation
ptab_mid <- ptable::create_cnt_ptable(D = 8,  V = 3.5, js = 2) # medium perturbation
ptab_hig <- ptable::create_cnt_ptable(D = 12, V = 5.0, js = 2) # large perturbation
...
# overview of noise distribution
plo <- ptab_low@pTable[ptab_low@pTable$i == max(ptab_low@pTable$i), ] %>%
  mutate(v = abs(v)) %>% group_by(v) %>% summarise(sum_p = round(sum(p), 2)) %>%
  mutate(csum_p = cumsum(sum_p))
pmi <- ptab_mid@pTable[ptab_mid@pTable$i == max(ptab_mid@pTable$i), ] %>%
  mutate(v = abs(v)) %>% group_by(v) %>% summarise(sum_p = round(sum(p), 2)) %>%
  mutate(csum_p = cumsum(sum_p))
phi <- ptab_hig@pTable[ptab_hig@pTable$i == max(ptab_hig@pTable$i), ] %>%
  mutate(v = abs(v)) %>% group_by(v) %>% summarise(sum_p = round(sum(p), 2)) %>%
  mutate(csum_p = cumsum(sum_p))
...
load("kernelheaping/dens_spop.RData")


## identify hotspots

hs_q90 <- hs_q50 <- dens_spop
qtl <- c(0.9, 0.5)

for(i in colnames(hs_q90)[4:57]) {
  
  for(j in versions) {
    
    dens <- dens_spop[dens_spop$version == j, i]
    
    hs_qtl1 <- quantile(dens[dens > 0], qtl[1])
    hs_qtl2 <- quantile(dens[dens > 0], qtl[2])
    
    in_hs1 <- dens_spop[dens_spop$version == j, i] >= hs_qtl1
    in_hs2 <- dens_spop[dens_spop$version == j, i] >= hs_qtl2
    
    hs_q90[hs_q90$version == j, i][in_hs1] <- 1
    hs_q90[hs_q90$version == j, i][!in_hs1] <- 0
    hs_q50[hs_q50$version == j, i][in_hs2] <- 1
    hs_q50[hs_q50$version == j, i][!in_hs2] <- 0
  }
}

## Calculate Jaccard similarities

jaccSim90 <- jaccSim50 <- data.frame(variable = rep(colnames(Berlin)[12:65], 3),
                                     version  = rep(c("low", "mid", "high"), each = 54))

# J(q) with q = 90%
jacc <- t(sapply(colnames(Berlin)[12:65], FUN = get_jaccard, data = hs_q90))
jaccSim90$jaccSim <- round(c(jacc[, 1], jacc[, 2], jacc[, 3]), 3)

# J(q) with q = 50%
jacc <- t(sapply(colnames(Berlin)[12:65], FUN = get_jaccard, data = hs_q50))
jaccSim50$jaccSim <- round(c(jacc[, 1], jacc[, 2], jacc[, 3]), 3)
...
ggplot() +
  geom_sf(data = Berlin) +
  geom_sf(data = hs_raster_to_sf(data = hs_q90, 
                                 var = "E_E65U80", 
                                 versions = c("original", "low", "high")) %>% arrange(desc(E_E65U80)),
          aes(color = E_E65U80, fill = E_E65U80), lty = "dotted", alpha = 0.3, lwd = 0.7) +
  ggtitle("Inhabitants between 65 and 80 years (Hotspots)", 
          subtitle = "10% most dense, Jaccard-sim.: 95-96%") +
  theme_void()
#ggsave(filename = "Hotspots_E_E65U80.png", width = 2400, height = 1200, units = "px")

ggplot() +
  geom_sf(data = Berlin) +
  geom_sf(data = hs_raster_to_sf(data = hs_q50,
                                 var = "HK_Turk", 
                                 versions = c("original", "low", "high")) %>% arrange(desc(HK_Turk)),
          aes(color = HK_Turk, fill = HK_Turk), lty = "dotted", alpha = 0.3, lwd = 0.7) +
  ggtitle("Inhabitants with Turkish migration background (Hotspots)", 
          subtitle = "50% most dense, Jaccard-sim.: 92%") +
  theme_void()
#ggsave(filename = "Hotspots_HK_Turk.png", width = 2400, height = 1200, units = "px")

ggplot(dens_spop) +
  geom_raster(aes(lon, lat, fill = E_E65U80), show.legend = FALSE) +
  scale_fill_gradientn(colours = c("#FFFFFF", "#5c87c2", "#19224e")) +
  geom_sf(data = Berlin, fill = NA) +
  facet_wrap(~version) +
  ggtitle("Spatial Density of inhabitants between 65 and 80 years",
          subtitle = "by level of CKM applied") +
  theme_void()
#ggsave(filename = "Density_E_E65U80.png", width = 2400, height = 2400, units = "px")
